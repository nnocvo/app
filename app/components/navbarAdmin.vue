...
<template>
    <nav class="bg-burdeo text-white shadow-md w-full z-50 relative border-b-4 border-gray-800">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex h-16 items-center justify-between">
                ...
                <div class="hidden md:flex items-center space-x-4 font-bold gap-2">
                    <span class="text-stone-300 text-sm">
                        Usuario: <strong class="text-white">{{ userSession?.nombre }} {{ userSession?.apellido
                            }}</strong>
                    </span>

                    <NuxtLink to="/admin/dashboard"
                        class="text-burdeo px-4 py-1.5 rounded-lg transition-all bg-white hover:bg-gray-100 text-sm">
                        Dashboard
                    </NuxtLink>

                    <button @click="cerrarSesion"
                        class="text-burdeo px-4 py-1.5 rounded-lg transition-all bg-white hover:bg-gray-100 text-sm cursor-pointer">
                        Cerrar Sesión
                    </button>
                </div>
        ...
        <div v-if="isMenuOpen" class="md:hidden border-t">
            <div class="px-4 pt-3 pb-4 space-y-3 font-bold flex flex-col">
                ...
                <NuxtLink to="/admin/dashboard" @click="isMenuOpen = false"
                    class="text-center bg-white text-burdeo px-4 py-2.5 rounded-xl transition-all hover:bg-gray-100 text-sm block">
                    Dashboard
                </NuxtLink>
